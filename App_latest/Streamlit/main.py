...
import sys, os,json
import traceback
from final.location import get_current_location
import pandas as pd

from final.route import get_top_dataframe
...
def initialize_chat_engine(index):
    try:
        ...
        if "chat_engine" not in st.session_state.keys():
            system_prompt = f"""As an AI system with access to Singapore's parking datasets, you're here to assist with queries related to car park availability, locations, agencies, and pricing. When you ask the following questions, accurate answers based on the requested location or context will be provided, even if specific details are not provided:
            "Show carparks in a given location/name/street."
            "Show car park availability (lots) - number of available lots."
            "Car park pricing."
            Please note that responses will include "I am not sure" if accurate information cannot be provided based on the available data or context..
             """""

            Settings.llm = OpenAI(model="gpt-3.5-turbo", temperature=0.0, system_prompt= system_prompt)
